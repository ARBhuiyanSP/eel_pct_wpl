 ...
 * *****************************************************************************
 */  
if (isset($_POST['consumption_submit']) && !empty($_POST['consumption_submit'])) {
	
	// check duplicate:
	$consumption_id	= $_POST['consumption_id'];
    $table		= 'inv_consumption';
    $where		= "consumption_id='$consumption_id'";
    if(isset($_POST['consumption_update_submit']) && !empty($_POST['consumption_update_submit'])){
        $notWhere   =   "id!=".$_POST['consumption_update_submit'];
		...
		$_SESSION['warning']    =   "Operation faild. Duplicate data found..!";
			...
			for ($count = 0; $count < count($_POST['quantity']); $count++) {
				...
				*/       
				...
				$query = "INSERT INTO `inv_consumptiondetails` (`consumption_id`,`consumption_date`,`material_id`,`material_name`,`unit`,`consumption_qty`,`consumption_price`,`part_no`,`project_id`,`warehouse_id`,`package_id`,`approval_status`) VALUES ('$consumption_id','$consumption_date','$material_id','$material_name','$unit','$quantity','0','$brand','$project_id','$warehouse_id','$package_id','0')";
				$conn->query($query);
				
				/*
				 *  Insert Data Into inv_materialbalance Table:
				*/
				$mb_ref_id      = $consumption_id;
				$mb_materialid  = $material_id;
				$mb_date        = (isset($consumption_date) && !empty($consumption_date) ? date('Y-m-d h:i:s', strtotime($consumption_date)) : date('Y-m-d h:i:s'));
				$mbin_qty       = 0;
				$mbin_val       = 0;
				$mbout_qty      = $quantity;
				$mbout_val      = 0;
			...
			*/
			$query2 = "INSERT INTO `inv_consumption` (`consumption_id`,`consumption_date`,`remarks`,`project_id`,`warehouse_id`) VALUES ('$consumption_id','$consumption_date','$remarks','$project_id','$warehouse_id')";
